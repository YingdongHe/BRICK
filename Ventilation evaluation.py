...
def _query_and_qualify(sensor):
    """
    Build query to return air supply measurements and qualify
    which site can run this application
    Parameters
    ----------
    sensor : sensor name type to evaluate e.g. Supply_Air_Flow
    Returns
    -------
...
def _fetch(qualify_resp, query, eval_start_time, eval_end_time, window):
    """
    Build the fetch query and define the building ventilation evaluation time.
    Parameters
    ----------
    qualify_resp : Mortar QualifyResponse object
    query : dictionary with query and sensor
    eval_start_time : start date and time in format (yyyy-mm-ddTHH:MM:SSZ) for the building
                      ventilation evaluation period
    eval_end_time : end date and time in format (yyyy-mm-ddTHH:MM:SSZ) for the building
                    ventilation evaluation period
...
def _clean(sensor, fetch_resp):
    ...
    Parameters
    ----------
    sensor : sensor name type to evaluate e.g. Supply_Air_Flow_Sensor
    fetch_resp : Mortar FetchResponse object
    Returns
    -------
...
def _analyze(query, fetch_resp, th_type, th_diff, th_time, window):
    """
    Parameters
    ----------
    query : dictionary with query and sensor
    ...
    for idx, equip in enumerate(equipment):
        ...
        # this increments every time we get a new run of sensor higher than the setpoint
        ...
        for g in lal.groups:
            ...
            if len(data) >= (60/th_time): # multiply by window frame to get hours
                fmt = {
                    ...
                    'diff': (data["{}_sensors".format(sensor)] - data["{}_sps".format(sensor)]).mean(),
                }
                records.append(fmt)
